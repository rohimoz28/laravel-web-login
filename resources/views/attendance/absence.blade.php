  ...
  <table class="table table-bordered">
    ...
    <tbody>
      <tr>
        ...
        <td>{{ (is_null($attendances->start)) ? 'Not Yet' : date("H:i:s", substr($attendances->start, 0, 10)) }}</td>
        <td>{{ (is_null($attendances->end)) ? 'Not Yet' : date("H:i:s", substr($attendances->end, 0, 10)) }}</td>
      </tr>
    </tbody>
  </table>
</div>
